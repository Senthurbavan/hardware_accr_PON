...
entity baseLocalPlanner_1_v1_0_S00_AXIS_PATH_DIST_MAP is
	generic (
...
end baseLocalPlanner_1_v1_0_S00_AXIS_PATH_DIST_MAP;

architecture arch_imp of baseLocalPlanner_1_v1_0_S00_AXIS_PATH_DIST_MAP is
	...
	constant NUMBER_OF_INPUT_WORDS  : integer := 1024;
	...
	-- BLOCKRAM Initialization
	type ram_type is array((NUMBER_OF_INPUT_WORDS-1) downto 0) of unsigned((C_S_AXIS_TDATA_WIDTH - 1) downto 0);
...
begin
	...
	-- Add user logic here
	WRITE : process(S_AXIS_ACLK)
	begin
		if rising_edge(S_AXIS_ACLK) then
			if(fifo_wren = '1') then
                RAM(write_pointer) := unsigned(S_AXIS_TDATA); --check wp and tdata
			else
				READ_DATA_0 <= RAM(to_integer(READ_ADD_0));
				READ_DATA_1 <= RAM(to_integer(READ_ADD_1));
				READ_DATA_2 <= RAM(to_integer(READ_ADD_2));
				READ_DATA_3 <= RAM(to_integer(READ_ADD_3));
				READ_DATA_4 <= RAM(to_integer(READ_ADD_4));
				READ_DATA_5 <= RAM(to_integer(READ_ADD_5));
            end if;
		end if;
	end process;
	-- User logic ends
